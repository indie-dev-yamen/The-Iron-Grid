extends Node2D
var hp:=values.mine_hp
var armor_d:=values.mine_armor
var armor_c:=values.mine_armor
var cost:=[values.mine_cost_food,values.mine_cost_wood,values.mine_cost_metal]
var needs_level:=0
var max:=10
var owner_id:=0
var pos:=Vector2(0,0)
var armor_broken_this_turn=false
var income:=values.mine_income
var res_id:=1

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	get_node("bg").texture=get_node("bg").texture.duplicate(true)
	res_id = get_parent().grid[pos.y][pos.x]
	match owner_id:
		1:
...
func turn_ended():
	get_parent().get_parent().resources[owner_id-1][res_id]+=income
		...
		for x in range(pos.x-1,pos.x+2):
			if get_parent().get_parent().grid2[y][x]:
				if get_parent().get_parent().grid2[y][x] == get_parent().get_parent().castles[owner_id-1]:
					found_castle=true
	if found_castle:
		get_parent().get_parent().resources[owner_id-1][res_id]+=get_parent().get_parent().castles[owner_id-1].bonus[get_parent().get_parent().castles[owner_id-1].level]
